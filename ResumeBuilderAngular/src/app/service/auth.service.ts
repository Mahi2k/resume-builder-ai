...
export class AuthService
{
    ...
    {
        return this.http.post<BasicObject>('http://localhost:4292/api/user/addUser',data).pipe(catchError(
    ...
    login(data:BasicObject):Observable<BasicObject>
        {
            return this.http.post<BasicObject>('http://localhost:4292/api/user/login',data).pipe(catchError(
                ()=>of({"success":false,"message":"An error occurred."})
                )).pipe(tap(
                    res=>{if(res["success"])
                    {
    ...
    {   console.log("REFRESHING USER")
        let subscription=this.http.get<{success:boolean,loggedIn?:boolean,data:AuthModel,message:string}>('http://localhost:4292/api/user/getLoggedIn').pipe(catchError(
    ...
    {
        return this.http.post<{success:boolean,message:string}>('http://localhost:4292/api/user/addSubscription',{subLevel:subLevel})
